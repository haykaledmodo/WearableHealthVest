"""
7-Fold Cross-Validation — CNN + Squeeze-Excitation Block  [BGL-2]
=========================================================================
Arsitektur : CNN baseline Mas Aldo (PPGBGL_v3) + Squeeze-Excitation Block
             setelah setiap Conv1D layer.
             SE Block: GAP → Dense(C//r) → ReLU → Dense(C) → Sigmoid → scale
...
Output  : Hasil_Eksperimen_BGL/BGL_2_CNN_SE/
"""
...
BATCH_SIZE  = 32
DROPOUT     = 0.3
SE_RATIO    = 4     # reduction ratio Squeeze-Excitation
SEED        = 42
...
OUT_DIR   = os.path.join(_SCRIPT_DIR, 'Hasil_Eksperimen_BGL', 'BGL_2_CNN_SE')
...
class SEBlock1D(keras.Layer):
    """Squeeze-Excitation Block untuk Conv1D."""
    def __init__(self, filters, ratio=4, **kwargs):
        super().__init__(**kwargs)
        self.filters=filters; self.ratio=ratio
        self.squeeze = keras.layers.GlobalAveragePooling1D()
        self.fc1     = keras.layers.Dense(max(1, filters//ratio), activation='relu')
        self.fc2     = keras.layers.Dense(filters, activation='sigmoid')
        self.reshape = keras.layers.Reshape((1, filters))
    def call(self, x):
        s = self.squeeze(x)
        s = self.fc1(s)
        s = self.fc2(s)
...
def cnn_se_block(x, filters, kernel_size, strides=1, pool_size=None, use_bn=False):
    x = keras.layers.Conv1D(filters, kernel_size=kernel_size,
                            strides=strides, activation='relu')(x)
    x = SEBlock1D(filters, ratio=SE_RATIO)(x)
    if use_bn:
...
def build_model():
    ...
    # Mirror arsitektur PPGBGL_v3 tapi setiap Conv diikuti SE Block
    x = cnn_se_block(inputs, 64, kernel_size=5, strides=2, pool_size=2, use_bn=True)
    x = cnn_se_block(x, 96, kernel_size=3)
    x = cnn_se_block(x, 208, kernel_size=3, pool_size=3)
    x = cnn_se_block(x, 64, kernel_size=3, pool_size=3)
    x = keras.layers.GlobalAveragePooling1D()(x)
    x = keras.layers.Dropout(DROPOUT)(x)
    outputs = keras.layers.Dense(1, name='glucose')(x)
    return keras.Model(inputs, outputs, name='PPGBGL_v3_SE')

# ─────────────────────────────────────────────────────────────────────────────
# Signal processing (identik BGL-1)
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    log('='*60)
    log('  BGL-2: CNN + Squeeze-Excitation Block — 7-Fold 27 subj')
    log('='*60)
    ...
    if all_records:
        ...
        log(f'\n  ╔══════════════════════════════════════════╗')
        log(f'  ║  RINGKASAN FINAL BGL-2 (27 subjek)')
        ...
        plot_clarke(df.gt_glucose.tolist(), df.pred_glucose.tolist(),
                    ...
                    title='Clarke EGA — BGL-2 CNN + SE Block')

        with open(METRIC_TXT, 'w', encoding='utf-8') as f:
            f.write('BGL-2: CNN + Squeeze-Excitation Block\n')
